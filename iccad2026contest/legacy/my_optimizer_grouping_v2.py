...
class MyOptimizer(FloorplanOptimizer):
    ...
    def solve(
        ...
        grouping_positions = self._repair_grouping_local(positions, cons, b2b_connectivity, p2b_connectivity, pins_pos)
        if not self._has_overlap(grouping_positions):
            return grouping_positions
        return positions
    ...
    def _repair_grouping_local(
        ...
        cluster_groups = self._groups(cons["cluster"][:len(positions)])
        broken = []
        for gid, members in cluster_groups.items():
            if any(cons["preplaced"][i] for i in members):
                continue
            components = self._component_count(members, positions)
            if components > 1:
                broken.append((components - 1, gid, members))
        ...
        for _, _, members in broken[:8]:
            candidate = self._candidate_group_repack(current, members, cons)
            if candidate is None or self._has_overlap(candidate):
                continue
            if self._accept_grouping_candidate(current, candidate, cons, b2b_connectivity, p2b_connectivity, pins_pos):
                current = candidate
        return current

    def _candidate_group_repack(
        self,
        positions: List[Tuple[float, float, float, float]],
        members: List[int],
        cons: Dict[str, List[int]],
    ) -> Optional[List[Tuple[float, float, float, float]]]:
        members = sorted(members, key=lambda i: positions[i][2] * positions[i][3], reverse=True)
        offsets: Dict[int, Tuple[float, float]] = {}
        x_cursor = 0.0
        group_h = 0.0
        for i in members:
            _, _, w, h = positions[i]
            offsets[i] = (x_cursor, 0.0)
            x_cursor += w
            group_h = max(group_h, h)
        group_w = x_cursor

        member_set = set(members)
        fixed_rects = [(x, y, w, h) for idx, (x, y, w, h) in enumerate(positions) if idx not in member_set]
        old_x = min(positions[i][0] for i in members)
        old_y = min(positions[i][1] for i in members)
        old_cx = sum(positions[i][0] + positions[i][2] / 2.0 for i in members) / len(members)
        old_cy = sum(positions[i][1] + positions[i][3] / 2.0 for i in members) / len(members)

        xs = {0.0, old_x, max(0.0, old_cx - group_w / 2.0)}
        ys = {0.0, old_y, max(0.0, old_cy - group_h / 2.0)}
        for x, y, w, h in fixed_rects:
            xs.add(x + w)
            xs.add(max(0.0, x - group_w))
            ys.add(y + h)
            ys.add(max(0.0, y - group_h))

        if len(xs) > 90:
            ranked = sorted(xs, key=lambda v: abs(v - old_x))
            xs = set(ranked[:60] + sorted(xs)[:20] + sorted(xs)[-10:])
        if len(ys) > 90:
            ranked = sorted(ys, key=lambda v: abs(v - old_y))
            ys = set(ranked[:60] + sorted(ys)[:20] + sorted(ys)[-10:])

        best_anchor = None
        best_score = float("inf")
        for x in sorted(xs):
            y = self._lowest_nonoverlap_y_rects(x, group_w, group_h, fixed_rects)
            for yy in (y, *list(sorted(ys)[:20])):
                if self._rect_overlaps_rects((x, yy, group_w, group_h), fixed_rects):
                    continue
                all_rects = fixed_rects + [(x, yy, group_w, group_h)]
                xmin = min(r[0] for r in all_rects)
                ymin = min(r[1] for r in all_rects)
                xmax = max(r[0] + r[2] for r in all_rects)
                ymax = max(r[1] + r[3] for r in all_rects)
                score = (xmax - xmin) * (ymax - ymin) + 0.2 * (abs(x - old_x) + abs(yy - old_y))
                if score < best_score:
                    best_score = score
                    best_anchor = (x, yy)
        ...
        for i in members:
            _, _, w, h = result[i]
            ox, oy = offsets[i]
            result[i] = (ax + ox, ay + oy, w, h)
        return result

    def _accept_grouping_candidate(
        self,
        before: List[Tuple[float, float, float, float]],
        after: List[Tuple[float, float, float, float]],
        cons: Dict[str, List[int]],
        ...
        b0, g0, m0, _ = self._soft_violations_fast(before, cons)
        b1, g1, m1, _ = self._soft_violations_fast(after, cons)
        if g1 >= g0:
            return False
        if b1 + m1 > b0 + m0:
            return False
        ...
        hpwl0 = calculate_hpwl_b2b(before, b2b_connectivity) + calculate_hpwl_p2b(before, p2b_connectivity, pins_pos)
        hpwl1 = calculate_hpwl_b2b(after, b2b_connectivity) + calculate_hpwl_p2b(after, p2b_connectivity, pins_pos)
        if hpwl1 > hpwl0 * (1.04 if g0 - g1 >= 2 else 1.01):
            return False

        return self._proxy_cost(after, cons, b2b_connectivity, p2b_connectivity, pins_pos) < (
            self._proxy_cost(before, cons, b2b_connectivity, p2b_connectivity, pins_pos) * 0.995
        )

    def _lowest_nonoverlap_y_rects(
        self,
        x: float,
        width: float,
        height: float,
