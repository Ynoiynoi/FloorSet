...
class MyOptimizer(FloorplanOptimizer):
    ...
    def _build_items(
        ...
        for _, members in sorted(self._groups(cons["cluster"]).items()):
            ...
            offsets, item_w, item_h = self._build_cluster_compact_layout(
                members, widths, heights, cons, target_positions
            )
            ...
            items.append(Item(members, offsets, item_w, item_h, fixed_anchor, bcode, pref, True))
    ...
    def _build_cluster_compact_layout(
        self,
        members: List[int],
        widths: List[float],
        heights: List[float],
        ...
        # Preplaced blocks make relative layout effectively fixed. Fall back to
        # the simple horizontal layout so the group anchor logic stays exact.
        if any(cons["preplaced"][b] for b in members):
            return self._build_cluster_row_layout(members, widths, heights)

        bits = 0
        for b in members:
            bits |= cons["boundary"][b]
        # Conflicting requests on the same axis are rare; keep the old safe path.
        if (bits & 1 and bits & 2) or (bits & 4 and bits & 8):
            return self._build_cluster_row_layout(members, widths, heights)

        total_area = sum(widths[b] * heights[b] for b in members)
        target_side = math.sqrt(max(total_area, 1.0))
        ordered = sorted(members, key=lambda b: widths[b] * heights[b], reverse=True)
        columns: List[List[int]] = []
        col_widths: List[float] = []
        col_heights: List[float] = []
        offsets: Dict[int, Tuple[float, float]] = {}

        # First-fit decreasing by column height to get a compact connected shape.
        for b in ordered:
            w, h = widths[b], heights[b]
            best_idx = -1
            best_score = float("inf")
            for idx in range(len(columns)):
                new_h = col_heights[idx] + h
                new_w = max(col_widths[idx], w)
                score = abs(new_h - target_side) + 0.3 * abs(new_w - target_side)
                if score < best_score:
                    best_score = score
                    best_idx = idx
            if best_idx == -1 or (col_heights[best_idx] > target_side * 1.1 and len(columns) < max(2, int(math.sqrt(len(members))))):
                columns.append([b])
                col_widths.append(w)
                col_heights.append(h)
            else:
                columns[best_idx].append(b)
                col_widths[best_idx] = max(col_widths[best_idx], w)
                col_heights[best_idx] += h

        x_cursor = 0.0
        total_w = 0.0
        total_h = max(col_heights) if col_heights else 0.0
        left_members = {b for b in members if cons["boundary"][b] & 1}
        right_members = {b for b in members if cons["boundary"][b] & 2}
        top_members = {b for b in members if cons["boundary"][b] & 4}
        bottom_members = {b for b in members if cons["boundary"][b] & 8}

        # Order columns so left/right boundary blocks naturally land on outer columns.
        def col_priority(col: List[int]) -> Tuple[int, int, float]:
            has_left = any(b in left_members for b in col)
            has_right = any(b in right_members for b in col)
            area = sum(widths[b] * heights[b] for b in col)
            return (-has_left, has_right, -area)

        columns = sorted(columns, key=col_priority)
        if right_members:
            right_cols = [col for col in columns if any(b in right_members for b in col)]
            other_cols = [col for col in columns if col not in right_cols]
            columns = other_cols + right_cols

        for col in columns:
            col_w = max(widths[b] for b in col)
            top = [b for b in col if b in top_members]
            bottom = [b for b in col if b in bottom_members and b not in top]
            middle = [b for b in col if b not in top and b not in bottom]
            y_bottom = 0.0
            y_top = total_h

            for b in sorted(bottom, key=lambda i: widths[i] * heights[i], reverse=True):
                offsets[b] = (x_cursor, y_bottom)
                y_bottom += heights[b]
            for b in sorted(top, key=lambda i: widths[i] * heights[i], reverse=True):
                y_top -= heights[b]
                offsets[b] = (x_cursor, y_top)
            for b in middle:
                offsets[b] = (x_cursor, y_bottom)
                y_bottom += heights[b]
        ...
        # Snap right-edge blocks to the item's right edge and top-edge blocks to top.
        for b in members:
            ox, oy = offsets[b]
            if b in right_members:
                ox = total_w - widths[b]
            if b in top_members:
                oy = total_h - heights[b]
            offsets[b] = (ox, oy)

        offsets = self._slide_cluster_components(members, offsets, widths, heights)
        min_x = min(offsets[b][0] for b in members)
        min_y = min(offsets[b][1] for b in members)
        if min_x != 0.0 or min_y != 0.0:
            offsets = {b: (x - min_x, y - min_y) for b, (x, y) in offsets.items()}
        total_w = max(offsets[b][0] + widths[b] for b in members)
        total_h = max(offsets[b][1] + heights[b] for b in members)
        return offsets, total_w, total_h

    def _slide_cluster_components(
        self,
        members: List[int],
        offsets: Dict[int, Tuple[float, float]],
        widths: List[float],
        ...
        original_components = len(self._cluster_components(members, original, widths, heights))
        original_area = self._rects_bbox_area(
            [(original[b][0], original[b][1], widths[b], heights[b], b) for b in members]
        )
        result = dict(offsets)
        for _ in range(len(members)):
            comps = self._cluster_components(members, result, widths, heights)
            if len(comps) <= 1:
                break
            comps.sort(key=lambda comp: sum(widths[b] * heights[b] for b in comp), reverse=True)
            fixed = set(comps[0])
            moved_any = False
            for comp in comps[1:]:
                move = self._best_axis_slide(comp, fixed, members, result, widths, heights)
                if move is None:
                    fixed.update(comp)
                    continue
                dx, dy = move
            ...
            if not moved_any:
                break
        final_rects = [(result[b][0], result[b][1], widths[b], heights[b], b) for b in members]
        for idx, rect in enumerate(final_rects):
            if self._rect_lists_overlap([rect], final_rects[idx + 1:]):
                return original
        final_components = len(self._cluster_components(members, result, widths, heights))
        final_area = self._rects_bbox_area(final_rects)
        if final_area > original_area + 1e-6:
            return original
        if final_components >= original_components and final_area >= original_area - 1e-6:
            return original
        return result
    ...
    def _cluster_components(
        ...
        for idx, a in enumerate(members):
            ax, ay = offsets[a]
            aw, ah = widths[a], heights[a]
            for b in members[idx + 1:]:
                bx, by = offsets[b]
                bw, bh = widths[b], heights[b]
    ...
    def _best_axis_slide(
        ...
        comp_rects = [(offsets[b][0], offsets[b][1], widths[b], heights[b], b) for b in comp]
        fixed_rects = [(offsets[b][0], offsets[b][1], widths[b], heights[b], b) for b in fixed]
        obstacle_rects = [(offsets[b][0], offsets[b][1], widths[b], heights[b], b) for b in members if b not in comp]
        before_components = len(self._cluster_components(members, offsets, widths, heights))
        before_area = self._rects_bbox_area(
            [(offsets[b][0], offsets[b][1], widths[b], heights[b], b) for b in members]
        )
        candidates = []

        comp_left = min(x for x, _, _, _, _ in comp_rects)
        comp_bottom = min(y for _, y, _, _, _ in comp_rects)
        comp_right = max(x + w for x, _, w, _, _ in comp_rects)
        comp_top = max(y + h for _, y, _, h, _ in comp_rects)
        fixed_left = min(x for x, _, _, _, _ in fixed_rects)
        fixed_bottom = min(y for _, y, _, _, _ in fixed_rects)
        fixed_right = max(x + w for x, _, w, _, _ in fixed_rects)
        fixed_top = max(y + h for _, y, _, h, _ in fixed_rects)

        x_overlap = min(comp_right, fixed_right) - max(comp_left, fixed_left)
        y_overlap = min(comp_top, fixed_top) - max(comp_bottom, fixed_bottom)
        if x_overlap > 1e-6:
            candidates.append((0.0, fixed_top - comp_bottom))
            candidates.append((0.0, fixed_bottom - comp_top))
        if y_overlap > 1e-6:
            candidates.append((fixed_right - comp_left, 0.0))
            candidates.append((fixed_left - comp_right, 0.0))
        ...
        for dx, dy in candidates:
            ...
            if key in seen:
                continue
            seen.add(key)
            moved = [(x + dx, y + dy, w, h, b) for x, y, w, h, b in comp_rects]
            if self._rect_lists_overlap(moved, obstacle_rects):
                continue
            trial_offsets = dict(offsets)
            for x, y, _, _, b in moved:
                trial_offsets[b] = (x, y)
            after_components = len(self._cluster_components(members, trial_offsets, widths, heights))
            if after_components >= before_components:
                continue
            after_area = self._rects_bbox_area(
                [(trial_offsets[b][0], trial_offsets[b][1], widths[b], heights[b], b) for b in members]
            )
            if after_area > before_area + 1e-6:
                continue
            improvement = before_components - after_components
            area_delta = before_area - after_area
            candidate_key = (-improvement, -area_delta, abs(dx) + abs(dy))
            if best_key is None or candidate_key < best_key:
                best_key = candidate_key
                best = (dx, dy)
        return best

    def _rect_lists_overlap(
        self,
        a_rects: List[Tuple[float, float, float, float, int]],
        b_rects: List[Tuple[float, float, float, float, int]],
    ) -> bool:
        for ax, ay, aw, ah, _ in a_rects:
            for bx, by, bw, bh, _ in b_rects:
                if min(ax + aw, bx + bw) - max(ax, bx) > 1e-6 and min(ay + ah, by + bh) - max(ay, by) > 1e-6:
                    return True
        return False

    def _rects_bbox_area(self, rects: List[Tuple[float, float, float, float, int]]) -> float:
        xmin = min(x for x, _, _, _, _ in rects)
        ymin = min(y for _, y, _, _, _ in rects)
        xmax = max(x + w for x, _, w, _, _ in rects)
        ymax = max(y + h for _, y, _, h, _ in rects)
