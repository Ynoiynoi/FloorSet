...
class MyOptimizer(FloorplanOptimizer):
    ...
    def _build_cluster_compact_layout(
        ...
        offsets = self._snap_cluster_components(members, offsets, widths, heights)
    ...
    def _snap_cluster_components(
        ...
        for _ in range(len(members)):
            ...
            moved_any = False
            for comp in comps[1:]:
                anchor = self._best_component_snap(comp, fixed, result, widths, heights)
                if anchor is None:
                    fixed.update(comp)
                    continue
                dx, dy = anchor
    ...
    def _best_component_snap(
        ...
        all_before = comp_rects + fixed_rects
        before_area = self._rects_bbox_area(all_before)

        for cx, cy, cw, ch, cb in comp_rects:
            for fx, fy, fw, fh, fb in fixed_rects:
                y_overlap_shift = [
                    fy - cy,
                    fy + fh - (cy + ch),
                ]
                for dy in y_overlap_shift:
                    ny1, ny2 = cy + dy, cy + dy + ch
                    if min(ny2, fy + fh) - max(ny1, fy) > 1e-6:
                        candidates.append((fx + fw - cx, dy))
                        candidates.append((fx - (cx + cw), dy))

                x_overlap_shift = [
                    fx - cx,
                    fx + fw - (cx + cw),
                ]
                for dx in x_overlap_shift:
                    nx1, nx2 = cx + dx, cx + dx + cw
                    if min(nx2, fx + fw) - max(nx1, fx) > 1e-6:
                        candidates.append((dx, fy + fh - cy))
                        candidates.append((dx, fy - (cy + ch)))
        ...
        for dx, dy in candidates:
            ...
            if self._rect_lists_overlap(moved, fixed_rects):
                continue
            after_area = self._rects_bbox_area(moved + fixed_rects)
            score = after_area + 0.05 * (abs(dx) + abs(dy))
            # Do not let snapping bloat the cluster too much.
            if after_area > before_area * 1.25:
                continue
            if score < best_score:
                best_score = score
                best = (dx, dy)
