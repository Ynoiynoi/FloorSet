...
6. All-soft grouping items compare a connected slicing-tree packing with
   the original chain and keep the better complete-layout candidate.
"""
...
class MyOptimizer(FloorplanOptimizer):
    def __init__(self, verbose: bool = False):
        ...
        self._group_slicing_enabled = True
    ...
    def solve(
        ...
        self._group_slicing_enabled = False
        chain_positions = self._solve_layout_variant(
            blocks,
            block_map,
            b2b_connectivity,
            p2b_connectivity,
        ...
        if not self._has_soft_group_slicing(blocks):
            self._group_slicing_enabled = True
            return chain_positions
        self._group_slicing_enabled = True
        slicing_positions = self._solve_layout_variant(
            blocks,
            block_map,
            b2b_connectivity,
            p2b_connectivity,
        ...
        return self._select_group_layout_variant(
            chain_positions, slicing_positions, blocks
        )

    def _solve_layout_variant(
        self,
    ...
    def _has_soft_group_slicing(self, blocks: Sequence[BlockSpec]) -> bool:
        groups: Dict[int, List[BlockSpec]] = {}
        for block in blocks:
            if block.group_id <= 0 or block.preplaced or block.boundary_code != 0:
                continue
            groups.setdefault(block.group_id, []).append(block)
        return any(
            len(members) >= 2 and all(not block.fixed for block in members)
            for members in groups.values()
        )

    def _select_group_layout_variant(
        self,
        chain_positions: List[Tuple[float, float, float, float]],
        slicing_positions: List[Tuple[float, float, float, float]],
        blocks: Sequence[BlockSpec],
    ) -> List[Tuple[float, float, float, float]]:
        if self._region_layout_key(dict(enumerate(chain_positions))) == self._region_layout_key(
            dict(enumerate(slicing_positions))
        ):
            return chain_positions

        chain_hpwl = self._layout_hpwl(chain_positions)
        slicing_hpwl = self._layout_hpwl(slicing_positions)
        chain_area = calculate_bbox_area(chain_positions)
        slicing_area = calculate_bbox_area(slicing_positions)
        chain_vrel = self._layout_soft_violation_ratio(chain_positions, blocks)
        slicing_vrel = self._layout_soft_violation_ratio(slicing_positions, blocks)

        # Convert changes relative to the chain candidate into approximate
        # baseline-relative gaps. The factors correspond to 1 + the current
        # typical HPWL/area gaps; the final exponential uses exact violations.
        quality_chain = 1.45
        quality_slicing = quality_chain + 0.5 * (
            1.8 * (slicing_hpwl / max(chain_hpwl, EPS) - 1.0)
            + 1.1 * (slicing_area / max(chain_area, EPS) - 1.0)
        )
        score_chain = quality_chain * math.exp(2.0 * chain_vrel)
        score_slicing = max(quality_slicing, 0.1) * math.exp(2.0 * slicing_vrel)
        if score_slicing + 1e-10 < score_chain:
            return slicing_positions
        return chain_positions
    ...
    def _layout_hpwl(
        ...
        total = 0.0
        for i, j, weight in self._current_b2b_edges:
        ...
        pins_pos = self._current_pins_pos
        if pins_pos is not None:
            for pin_idx, bid, weight in self._current_p2b_edges:
                if bid >= len(centers) or pin_idx >= len(pins_pos):
                    continue
                total += weight * (
                    abs(centers[bid][0] - float(pins_pos[pin_idx][0]))
                    + abs(centers[bid][1] - float(pins_pos[pin_idx][1]))
                )
        return total

    def _layout_soft_violation_ratio(
        self,
        positions: Sequence[Tuple[float, float, float, float]],
        blocks: Sequence[BlockSpec],
    ) -> float:
        ...
        for block in blocks:
            if block.group_id > 0:
                groups.setdefault(block.group_id, []).append(block.block_id)
            if block.mib_id > 0:
                mib_groups.setdefault(block.mib_id, []).append(block.block_id)
            if block.boundary_code != 0:
                boundary_ids.append(block.block_id)

        n_soft = len(boundary_ids)
        n_soft += sum(max(0, len(ids) - 1) for ids in groups.values())
        n_soft += sum(max(0, len(ids) - 1) for ids in mib_groups.values())

        x0, y0, x1, y1 = self._bbox_xyxy(positions)
        violations = 0
        for bid in boundary_ids:
            x, y, w, h = positions[bid]
            code = blocks[bid].boundary_code
            touches = {
                1: abs(x - x0) < 1e-6,
                2: abs(x + w - x1) < 1e-6,
                4: abs(y + h - y1) < 1e-6,
        ...
        def connected(first: int, second: int) -> bool:
            ax, ay, aw, ah = positions[first]
            bx, by, bw, bh = positions[second]
            overlap_x = min(ax + aw, bx + bw) - max(ax, bx)
            overlap_y = min(ay + ah, by + bh) - max(ay, by)
            vertical_touch = (
                abs(ax + aw - bx) < 1e-6 or abs(bx + bw - ax) < 1e-6
            ) and overlap_y > 1e-7
            horizontal_touch = (
                abs(ay + ah - by) < 1e-6 or abs(by + bh - ay) < 1e-6
            ) and overlap_x > 1e-7
            return vertical_touch or horizontal_touch or (
                overlap_x > 1e-7 and overlap_y > 1e-7
            )

        for ids in groups.values():
            if len(ids) < 2:
                continue
            if shapely_box is not None and shapely_union is not None:
                polygons = [
                    shapely_box(
                        positions[bid][0],
                        positions[bid][1],
                        positions[bid][0] + positions[bid][2],
                        positions[bid][1] + positions[bid][3],
                    )
                    for bid in ids
                ]
                merged = shapely_union(polygons)
                if merged.geom_type == "MultiPolygon":
                    violations += len(merged.geoms) - 1
                continue
            adjacency: Dict[int, List[int]] = {bid: [] for bid in ids}
            ...
            for start in ids:
                ...
                while stack:
                    ...
                    if bid in seen:
                        continue
                    seen.add(bid)
                    stack.extend(adjacency[bid])
            violations += max(0, components - 1)

        for ids in mib_groups.values():
            shapes = {
                (round(positions[bid][2], 4), round(positions[bid][3], 4))
                for bid in ids
            }
    ...
    def _make_group_item(
        ...
        if all_soft:
            best_local = self._make_soft_group_slicing_layout(members)
    ...
    def _make_soft_group_slicing_layout(
        self,
        members: Sequence[BlockSpec],
    ) -> Dict[int, Tuple[float, float, float, float]]:
        ids = [block.block_id for block in members]
        block_map = self._current_block_map
        total_area = sum(block.area for block in members)
        root_h = math.sqrt(max(total_area, 1.0))
        chain_order = sorted(members, key=lambda item: (-item.area, item.block_id))
        root_w = sum(block.area / root_h for block in chain_order)
        guides = self._group_soft_guides(ids)
        member_set = set(ids)
        internal_b2b = [
            edge for edge in self._current_b2b_edges
            if edge[0] in member_set and edge[1] in member_set
        ]

        # Preserve the original area-sorted horizontal chain as the fallback.
        chain: Dict[int, Tuple[float, float, float, float]] = {}
        cursor = 0.0
        for block in chain_order:
            width = block.area / root_h
            chain[block.block_id] = (cursor, 0.0, width, root_h)
            cursor += width

        if not self._group_slicing_enabled:
            return chain

        candidates = [chain]
        trees = self._build_slice_tree_variants(ids, block_map, guides, internal_b2b)
        region = FreeRect(0.0, 0.0, root_w, root_h)
        seen = {self._region_layout_key(chain)}
        for tree in trees:
            layout: Dict[int, Tuple[float, float, float, float]] = {}
            self._materialize_slice_tree(tree, 0.0, 0.0, root_w, root_h, layout)
            for transformed in self._rect_layout_reflections(layout, root_w, root_h):
                key = self._region_layout_key(transformed)
                if key in seen:
                    continue
                seen.add(key)
                if self._validate_region_layout(region, ids, transformed, block_map):
                    candidates.append(transformed)

        best = chain
        best_score = self._score_soft_group_layout(chain, ids)
        for layout in candidates[1:]:
            score = self._score_soft_group_layout(layout, ids)
            if score + 1e-10 < best_score:
                best = layout
                best_score = score
        return best

    def _group_soft_guides(
        self,
        block_ids: Sequence[int],
    ) -> Dict[int, Tuple[float, float, float]]:
        member_set = set(block_ids)
        sums: Dict[int, List[float]] = {
            bid: [0.0, 0.0, 0.0] for bid in block_ids
        }
        pins_pos = self._current_pins_pos
        if pins_pos is not None:
            for pin_idx, bid, weight in self._current_p2b_edges:
                if bid not in member_set or pin_idx >= len(pins_pos):
                    continue
        ...
        for i, j, weight in self._current_b2b_edges:
            if i in member_set and j in member_set:
                ...
                continue
            inside = i if i in member_set else j if j in member_set else None
            outside = j if inside == i else i if inside == j else None
            if inside is None or outside is None:
                continue
            block = self._current_block_map.get(outside)
            if block is None or not block.preplaced:
                continue
            cx = block.x + 0.5 * block.width
            cy = block.y + 0.5 * block.height
            sums[inside][0] += cx * weight
            sums[inside][1] += cy * weight
            sums[inside][2] += weight

        guides: Dict[int, Tuple[float, float, float]] = {}
        for bid, (sx, sy, sw) in sums.items():
            if sw > EPS:
                guides[bid] = (sx / sw, sy / sw, sw)

        # Propagate anchor coordinates through internal connectivity so an
        # unpinned chain still follows anchored members of the same group.
        for _ in range(len(block_ids)):
            ...
            for bid in block_ids:
                ...
                linked = [
                    (guides[other], weight)
                    for other, weight in adjacency[bid]
                    if other in guides
                ]
                if not linked:
                    continue
                total_weight = sum(max(weight, EPS) for _, weight in linked)
                gx = sum(guide[0] * max(weight, EPS) for guide, weight in linked) / total_weight
                gy = sum(guide[1] * max(weight, EPS) for guide, weight in linked) / total_weight
                updates[bid] = (gx, gy, total_weight)
            if not updates:
                break
            guides.update(updates)

        fallback_order = sorted(
            block_ids,
            key=lambda bid: (-len(adjacency[bid]), -self._current_block_map[bid].area, bid),
        )
        for rank, bid in enumerate(fallback_order):
            if bid not in guides:
                guides[bid] = (float(rank), 0.0, 1.0)
        return guides

    def _rect_layout_reflections(
        self,
        layout: Dict[int, Tuple[float, float, float, float]],
        root_w: float,
        root_h: float,
    ) -> List[Dict[int, Tuple[float, float, float, float]]]:
        result: List[Dict[int, Tuple[float, float, float, float]]] = []
        seen = set()
        for mirror_x in (False, True):
            for mirror_y in (False, True):
                transformed: Dict[int, Tuple[float, float, float, float]] = {}
                for bid, (x, y, w, h) in layout.items():
                    if mirror_x:
                        x = root_w - x - w
                    if mirror_y:
                        y = root_h - y - h
                    transformed[bid] = (x, y, w, h)
                key = self._region_layout_key(transformed)
                if key in seen:
                    continue
                seen.add(key)
    ...
    def _score_soft_group_layout(
        ...
        for i, j, weight in self._current_b2b_edges:
            if i in member_set and j in member_set:
                ix, iy = center(i)
                jx, jy = center(j)
                wire_cost += weight * (abs(ix - jx) + abs(iy - jy))
                total_weight += weight
                continue
            inside = i if i in member_set else j if j in member_set else None
            outside = j if inside == i else i if inside == j else None
            if inside is None or outside is None:
                continue
            block = self._current_block_map.get(outside)
            if block is not None and block.preplaced:
                anchors.append((
                    inside,
        ...
        if pins_pos is not None:
            for pin_idx, bid, weight in self._current_p2b_edges:
                if bid in member_set and pin_idx < len(pins_pos):
                    anchors.append((
                        bid,
                        float(pins_pos[pin_idx][0]),
                        float(pins_pos[pin_idx][1]),
                        weight,
                    ))

        if anchors:
            x_shifts = [(ax - center(bid)[0], weight) for bid, ax, _, weight in anchors]
            y_shifts = [(ay - center(bid)[1], weight) for bid, _, ay, weight in anchors]
            tx = self._weighted_median(x_shifts)
            ty = self._weighted_median(y_shifts)
            for bid, ax, ay, weight in anchors:
                cx, cy = center(bid)
                wire_cost += weight * (abs(cx + tx - ax) + abs(cy + ty - ay))
                total_weight += weight

        total_area = sum(self._current_block_map[bid].area for bid in block_ids)
        scale = max(total_weight * math.sqrt(max(total_area, EPS)), 1.0)
        shape_penalty = 0.0
        extreme_penalty = 0.0
        for _, _, w, h in layout.values():
            log_ratio = math.log(max(w, EPS) / max(h, EPS))
            shape_penalty += log_ratio * log_ratio
            ratio = max(w / max(h, EPS), h / max(w, EPS))
            if ratio > 12.0:
                extreme_penalty += ((ratio - 12.0) / 12.0) ** 2
        count = max(len(layout), 1)
        return (
            wire_cost / scale
            + 0.02 * shape_penalty / count
            + 0.05 * extreme_penalty / count
        )
