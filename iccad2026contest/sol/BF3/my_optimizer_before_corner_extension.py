...
class MyOptimizer(FloorplanOptimizer):
    ...
    def _place_boundary_blocks(
        ...
        corner_dims, extents = self._choose_boundary_geometry(
            corners,
            core_w,
            core_h,
            left_blocks,
    ...
    def _choose_boundary_geometry(
        ...
        def rec(idx: int, current: Dict[int, Tuple[float, float]]) -> None:
            nonlocal best_dims, best_extents, best_score
            if idx == len(codes):
        ...
        rec(0, {})
        if best_dims is None or best_extents is None:
            fallback = {code: choices[code][0] for code in codes}
            return fallback, self._solve_boundary_extents(
                fallback,
                core_w,
                core_h,
                left_blocks,
