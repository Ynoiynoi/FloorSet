...
### 17.1 具体形状是被方向和条带厚度唯一确定的

设一个面积为 `A` 的 soft corner 选择沿 top/bottom 水平延伸，最终水平条带厚度为 `H`。

若角块高度小于 `H`，将其高度增大到 `H` 不会增加外框厚度，却会把宽度从 `A / h` 降到 `A / H`，从而释放沿边空间。因此原形状被严格支配。最优形状必然为：
...
所以固定延伸方向后，不需要再枚举正方形、旋转形状或边厚匹配形状；最终条带厚度会唯一确定角块尺寸。

### 17.2 四边联合固定点

当前版本只枚举四个角的 `horizontal/vertical` 方向，包括空角区应分配给哪一条相邻边：
...
1. horizontal corner 的面积并入对应 top/bottom soft area。
2. vertical corner 的面积并入对应 left/right soft area。
3. 根据当前 `L/R/T/B` 计算四条边剩余长度。
4. 用 `soft_area / remaining_length` 更新四条条带厚度。
5. 对固定模块长度不足的边增加可用方向上的扩展。
6. 使用 0.5 阻尼迭代，消除水平和垂直方程间可能出现的二周期。
7. 收敛后通过 `area / strip_thickness` 反算全部 soft corner 尺寸。

模式对应的角区只分配给一条边：horizontal 模式允许水平边使用角区，并令垂直边避开完整水平条带；vertical 模式反之。这保证两组边模块不会在角区重叠。

### 17.3 复杂度

设 boundary 模块数为 `n`。旧版本最多枚举 `10^4` 个形状与方向组合，每个组合做 10 次线性扫描：

```text
旧几何求解：O(10^5 n)
```

新版本固定枚举 16 个方向组合，每个组合最多做 96 次线性固定点迭代：

```text
新几何求解：O(16 * 96 * n) = O(n)
```

最终 boundary 两两 overlap safety check 仍为 `O(n^2)`，所以完整 boundary 流程通常可写为 `O(n^2 + E)`；其中 `E` 是连接目标的扫描量。几何求解的常数约下降两个数量级。

### 17.4 case 97

```text
指标                    形状枚举版       仅方向版
local cost              1.8478           1.7747
HPWLgap                 0.8188           0.8014
Areagap                 0.0117           0.0093
Vrel                    0.1333           0.1167
bbox area               37816.97         37727.96
单次 solve runtime      1.48s            0.59s
```

块 32 的尺寸为 `25.0000 x 23.9200`，块 39 的尺寸为 `16.5903 x 34.7190`；两者都由最终左右条带宽度直接反算。
...
指标                         形状枚举版      仅方向版
Feasible                     100 / 100       100 / 100
正式 Total Score             1.7886          1.7620
正式 Avg Cost                1.9200          1.8554
正式 Avg Runtime             1.12s           0.32s
HPWLgap                      0.807605        0.790800
Areagap_bbox                 0.142459        0.105063
Violationsrelative           0.129294        0.122025
Vboundary/Nsoft              0.028333        0.028333
Vgrouping/Nsoft              0.100961        0.093692
Vmib/Nsoft                   0.000000        0.000000
grouping violation units     438             410
```

方向固定点版本同时降低了时间、面积、HPWL 和 grouping violation，因此替换为当前主版本。

## 18. Grouping 软链的局部 slicing tree

### 18.1 适用范围

当前 grouping item 在以下条件全部满足时生成 slicing 候选：

1. block 不是 preplaced。
2. block 不带 boundary；boundary + grouping 仍优先 boundary。
3. 过滤后 grouping item 至少有两个成员。
4. item 内全部成员都是可变形 soft block。

含 fixed block 或已经应用 MIB template 的 grouping item 继续使用原横/纵链，不允许 slicing tree 改变硬尺寸。

### 18.2 连通 slicing root

原 all-soft grouping item 使用公共高度形成一条横链。新版本保留完全相同的横链，同时生成若干递归 H/V slicing tree：

```text
group members
    -> pin / preplaced-neighbor guides
    -> internal b2b guided cut
    -> H/V alternating slicing trees
    -> subtree area-ratio materialization
    -> horizontal/vertical reflection variants
```

每个内部节点把父矩形完整切成两个子矩形；两个子树沿切线具有公共边，所有叶子并集恰好等于根矩形。因此每个 slicing candidate 天然是单一连通分量，满足 grouping item 内部连续相接的要求。

根矩形宽高严格沿用旧横链的浮点计算路径：高度为 `sqrt(total_area)`，宽度通过逐块累加 `area_i / height` 得到。这样 slicing 只改变 item 内部结构，不因末位浮点差异改变外层刚性 item 排序。
...
1. 组内 b2b 中心曼哈顿距离。
2. p2b pin 和外部 preplaced 邻居形成的锚点距离。
3. 对整个 item 的平移用加权中位数消去，使评分只比较内部相对结构。
4. 普通长宽比惩罚和极端细长惩罚。
...
局部代理无法预测 boundary 后处理和最终 overlap safety pass 对其他残缺 grouping 组的影响，因此主程序会构造两份完整布局：

```text
candidate A: 所有 all-soft grouping item 使用原横链
candidate B: 每个 all-soft grouping item 使用局部最优 slicing/横链
```
...
- bbox area；
- 全部 b2b/p2b HPWL；
- 与官方一致的 boundary violation；
- Shapely `unary_union` grouping connected components；
- MIB distinct shapes。

只有 candidate B 的完整布局代理分数更小时才采用 slicing，否则逐坐标返回原横链版本。最终验证集中有 40 个案例采用 slicing，40 个案例 cost 全部下降，没有逐例回退。
...
```text
指标                         原横链版         Group slicing
Feasible                     100 / 100       100 / 100
正式 Total Score             1.7620          1.7509
正式 Avg Cost                1.8554          1.8345
正式 Avg Runtime             0.32s           0.71s
HPWLgap                      0.790800        0.779792
Areagap_bbox                 0.105063        0.104341
Violationsrelative           0.122025        0.118507
Vboundary/Nsoft              0.028333        0.028333
Vgrouping/Nsoft              0.093692        0.090173
Vmib/Nsoft                   0.000000        0.000000
grouping violation units     410             393
```

原方向固定点版本及对应结果备份为：

- `my_optimizer_corner_direction_fixedpoint.py`
- `result_corner_direction_fixedpoint.txt`
- `analyze_corner_direction_fixedpoint.txt`
- `sol_corner_direction_fixedpoint.md`

结论：grouping slicing tree 提高了组内二维结构表达能力；完整布局双候选消除了局部代理造成的回退，代价是求解时间约翻倍。
