...
class MyOptimizer(FloorplanOptimizer):
    ...
    def _choose_boundary_geometry(
        ...
        left_est, _ = self._vertical_side_requirements(left_blocks, max(core_h, 1e-6))
        right_est, _ = self._vertical_side_requirements(right_blocks, max(core_h, 1e-6))
        top_est, _ = self._horizontal_side_requirements(top_blocks, max(core_w, 1e-6))
        bottom_est, _ = self._horizontal_side_requirements(bottom_blocks, max(core_w, 1e-6))
        side_width = {5: left_est, 9: left_est, 6: right_est, 10: right_est}
        side_height = {5: top_est, 6: top_est, 9: bottom_est, 10: bottom_est}

        choices: Dict[int, List[Tuple[float, float]]] = {}
        core_ratio = max(core_w, 1.0) / max(core_h, 1.0)
        for code, block in corners.items():
            if block is None:
            ...
            width = math.sqrt(max(block.area * core_ratio, 1e-6))
            height = block.area / max(width, EPS)
            side = math.sqrt(max(block.area, 1e-6))
            raw = [(width, height), (height, width), (side, side)]
            if side_width[code] > EPS:
                raw.append((side_width[code], block.area / side_width[code]))
            if side_height[code] > EPS:
                raw.append((block.area / side_height[code], side_height[code]))
            ...
            for w, h in raw:
                if w <= EPS or h <= EPS:
                    continue
                key = (round(w, 7), round(h, 7))
                if key in seen:
                    continue
                seen.add(key)
                deduped.append((w, h))
            choices[code] = deduped
        ...
        def rec(
            ...
            nonlocal best_dims, best_modes, best_extents, best_score
            if idx == len(codes):
                extents = self._solve_boundary_extents(
                    current,
                    current_modes,
                    core_w,
                    core_h,
            ...
            for dims in choices[code]:
                current[code] = dims
                modes = ('horizontal',) if dims == (0.0, 0.0) else ('horizontal', 'vertical')
                for mode in modes:
                    current_modes[code] = mode
                    rec(idx + 1, current, current_modes)

        rec(0, {}, {})
        if best_dims is None or best_modes is None or best_extents is None:
            fallback = {code: choices[code][0] for code in codes}
            fallback_modes = {
                code: 'horizontal' if dims[0] >= dims[1] else 'vertical'
                for code, dims in fallback.items()
            }
            return fallback, fallback_modes, self._solve_boundary_extents(
                fallback,
                fallback_modes,
                core_w,
                core_h,
    ...
    def _solve_boundary_extents(
        ...
        w_bl, h_bl = corner_dims.get(9, (0.0, 0.0))
        w_br, h_br = corner_dims.get(10, (0.0, 0.0))
        mode_tl = corner_modes.get(5, 'horizontal')
        mode_tr = corner_modes.get(6, 'horizontal')
        mode_bl = corner_modes.get(9, 'horizontal')
        mode_br = corner_modes.get(10, 'horizontal')

        # A corner only needs to stay outside the core in one direction.
        # Horizontal mode lets it grow along top/bottom; vertical mode lets it
        # grow along left/right. Requiring both dimensions outside the core
        # creates unnecessarily large empty corner rectangles.
        left_extra = max(
            w_tl if mode_tl == 'vertical' else 0.0,
            w_bl if mode_bl == 'vertical' else 0.0,
        )
        right_extra = max(
            w_tr if mode_tr == 'vertical' else 0.0,
            w_br if mode_br == 'vertical' else 0.0,
        )
        top_extra = max(
            h_tl if mode_tl == 'horizontal' else 0.0,
            h_tr if mode_tr == 'horizontal' else 0.0,
        )
        bottom_extra = max(
            h_bl if mode_bl == 'horizontal' else 0.0,
            h_br if mode_br == 'horizontal' else 0.0,
        )
        ...
        for _ in range(10):
            ...
            total_w = core_w + L + R
            total_h = core_h + T + B
            gap_left = max(total_h - eff_h_tl - eff_h_bl, 1e-6)
            gap_right = max(total_h - eff_h_tr - eff_h_br, 1e-6)
            gap_top = max(total_w - eff_w_tl - eff_w_tr, 1e-6)
            gap_bottom = max(total_w - eff_w_bl - eff_w_br, 1e-6)
            ...
            # Opposite corners on an edge must fit without intersecting.
            for pair_w, left_mode, right_mode in (
                (w_tl + w_tr, mode_tl, mode_tr),
                (w_bl + w_br, mode_bl, mode_br),
            ):
                shortage = pair_w - (core_w + newL + newR)
                if shortage <= EPS:
                    continue
                left_flexible = left_mode == 'horizontal'
                right_flexible = right_mode == 'horizontal'
                if left_flexible and right_flexible:
                    newL += 0.5 * shortage
                    newR += 0.5 * shortage
                elif left_flexible:
                    newL += shortage
            ...
            for pair_h, top_mode, bottom_mode in (
                (h_tl + h_bl, mode_tl, mode_bl),
                (h_tr + h_br, mode_tr, mode_br),
            ):
                shortage = pair_h - (core_h + newT + newB)
                if shortage <= EPS:
                    continue
                top_flexible = top_mode == 'vertical'
                bottom_flexible = bottom_mode == 'vertical'
                if top_flexible and bottom_flexible:
                    newT += 0.5 * shortage
                    newB += 0.5 * shortage
                elif top_flexible:
                    newT += shortage
                elif bottom_flexible:
                    newB += shortage
                else:
                    newT += 0.5 * shortage
                    newB += 0.5 * shortage

            if max(abs(newL - L), abs(newR - R), abs(newT - T), abs(newB - B)) < 1e-6:
