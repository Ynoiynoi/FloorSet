#!/usr/bin/env python3
"""
PeFsol1: strip soft constraints into rigid local structures, then solve the
remaining hard-constraint core with a PeF-style global placement flow.
"""
...
class MyOptimizer(FloorplanOptimizer):
    ...
    def solve(
        ...
        core_items, boundary_blocks = self._build_core_items(blocks, group_items, grouped_ids)
        ...
        if core_items:
            ...
            x, y, _, _ = legalize_with_obstacle_aware_graphs(
                x,
                y,
                w,
                h,
                outline_w,
                outline_h,
                np.array([item.fixed_position for item in core_items], dtype=bool),
                np.array([item.fixed_x for item in core_items], dtype=float),
                np.array([item.fixed_y for item in core_items], dtype=float),
            )
        ...
        placements = self._place_boundary_blocks(blocks, core_positions, boundary_blocks)
        result = [placements[i] for i in range(block_count)]
    ...
    # ------------------------------------------------------------------
    # Input parsing and soft stripping
    # ------------------------------------------------------------------
    def _build_blocks(
        self,
        block_count: int,
    ...
    def _build_core_items(
        self,
        blocks: Sequence[BlockSpec],
        group_items: Sequence[LayoutItem],
        grouped_ids: set[int],
    ) -> Tuple[List[LayoutItem], List[BlockSpec]]:
        items: List[LayoutItem] = list(group_items)
        boundary_blocks: List[BlockSpec] = []
    ...
    def _estimate_outline(
        ...
        outline_w = max(outline_w, 2.0 * base_side)
        outline_h = max(outline_h, 2.0 * base_side)
    ...
    def _legalize_final_blocks(
        ...
        centers_x, centers_y, _, _ = legalize_with_obstacle_aware_graphs(
            centers_x,
            centers_y,
            widths,
            heights,
